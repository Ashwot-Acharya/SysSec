...
class Sequitur:
    """
    Learns a context-free grammar from a sequence of symbols.

    Usage
    -----
        sq = Sequitur()
        sq.learn(['open','read','read','close','open','read','close'])
        sq.print_grammar()

    The grammar is stored as:
        self.start_rule  — the top-level rule S whose body IS the working string
        self.rules       — dict { rule_id: Rule }
        self.digram_index — dict { (key1, key2): Symbol }
                           maps each digram to the FIRST of the two symbols
                           (so we know where it lives to replace it)
    """
...
def test_toy_abcabd():
    """
    Classic textbook example from the original SEQUITUR paper.
    Input:  a b c a b d a b c
    Expected: S → R2 R1 d R2
              R1 → a b
              R2 → R1 c
    """
    print("\n" + "━"*50)
    print("TEST 1: Classic paper example  (a b c a b d a b c)")
    print("━"*50)

    seq = list("abcabdabc")   # ['a','b','c','a','b','d','a','b','c']
    sq = SequiturSimple()
    sq.learn(seq)
    sq.print_grammar()

    expanded = sq.expand()
    original = list("abcabdabc")
    match = expanded == original
    print(f"\n  Expansion matches original: {match}")
    print(f"  Expanded: {''.join(expanded)}")
    print(f"  Original: {''.join(original)}")


def test_syscall_toy():
    """
    Syscall-like sequence — closer to your actual use case.
    Normal program pattern: open read* close  repeated several times.
    """
    print("\n" + "━"*50)
    print("TEST 2: Syscall toy sequence")
    print("━"*50)
    ...
    print(f"  Input ({len(seq)} symbols): {' '.join(seq)}")
...
def test_anomaly_detection():
    """
    Show that SEQUITUR builds a grammar on normal data,
    and that an anomalous sequence does NOT fit the grammar
    (it would have unknown digrams / rule mismatch).

    For now we just show the grammar learned — the PCFG + Inside
    algorithm (next step) will give the actual probability score.
    """
    print("\n" + "━"*50)
    print("TEST 3: Normal vs Anomalous sequences")
    print("━"*50)

    # Training corpus — normal file-access patterns
    normal_traces = [
        ['open','read','write','close'],
        ['open','read','close'],
        ['open','write','close'],
        ['open','read','read','write','close'],
        ['open','read','write','write','close'],
        ['open','read','close'],
    ]

    # Flatten all traces into one long sequence for grammar induction
    full_seq = []
    for trace in normal_traces:
        full_seq.extend(trace)
        full_seq.append('||')   # trace separator — acts as a boundary marker

    print(f"  Training on {len(normal_traces)} normal traces ({len(full_seq)} symbols total)")

    sq = SequiturSimple()
    sq.learn(full_seq)
    sq.print_grammar()

    # Show what symbols the grammar knows about
    grammar = sq.get_grammar()
    all_terminals = set()
    for body in grammar.values():
        for sym in body:
            if not sym.startswith('R') and sym != 'S':
                all_terminals.add(sym)

    print(f"\n  Known terminals: {sorted(all_terminals)}")

    # Anomalous sequences contain unexpected patterns
    test_seqs = [
        (['open','read','write','close'],       "normal"),
        (['open','write','read','close'],        "suspicious (write before read)"),
        (['open','mmap','execve','connect'],     "ATTACK (shellcode pattern)"),
        (['connect','send','recv','close'],      "suspicious (no open)"),
    ]

    print("\n  Anomaly check (unknown symbol = anomalous indicator):")
    for test_seq, label in test_seqs:
        unknown = [s for s in test_seq if s not in all_terminals and not s.startswith('R')]
        status = "⚠️  UNKNOWN SYMBOLS" if unknown else "✓  known pattern"
        print(f"    {label:<45} {status}  {unknown if unknown else ''}")


def test_nested_structure():
    """
    Prove context-free expressiveness:
    The nested open/close pattern  open^n close^n  is NOT regular.
    SEQUITUR captures the recursive structure; an n-gram model cannot.
    """
    print("\n" + "━"*50)
    print("TEST 4: Nested bracket structure  (open^n close^n)")
    print("━"*50)

    # Generate open^n close^n for n = 1..4
    seq = []
    for n in range(1, 5):
        seq.extend(['open'] * n + ['close'] * n + ['||'])
    ...
    grammar = sq.get_grammar()
    print("\n  Grammar captures recursive open/close pairing.")
    print("  A bigram (n-gram) model would just see co-occurrence frequencies")
    print("  and CANNOT enforce that every open has a matching close.")
    print("  This is provable via the Pumping Lemma for Regular Languages.")
...
if __name__ == "__main__":
    print()
    print("╔══════════════════════════════════════════════╗")
    print("║   SEQUITUR Algorithm — Verification Suite   ║")
    print("╚══════════════════════════════════════════════╝")
    ...
    print("\n\nAll tests complete.")
    print("Next step: feed grammar into PCFG (count rule frequencies → normalize).")
    print("Then: Inside (CYK) algorithm for anomaly scoring.")
